 ...
 *
 * g++ -std=c++17 -DUSE_LAPACK -I.. -I../.. -I/usr/include/lapacke -I/usr/local/opt/lapack/include -o eig17 eig17.cpp ../libs/log.cpp -L/usr/local/opt/lapack/lib -L/usr/local/lib -llapacke
 *
 * ----------------------------------------------------------------------------
 * tlibs
...
void set_col(std::size_t failures)
{
	if(failures == 0)
		std::cerr << "\x1b[1;32m";
	else
...
int main()
{
	...
	// there is a bug in older versions of lapacke: https://github.com/Reference-LAPACK/lapack/issues/379
	std::cout << "\nHermitian" << std::endl;
	failures = tst_herm();
	total_failures += failures;
	set_col(failures);
